...
namespace TestSklad2
{
    class Program
    {
        ...
        {            
            SkladWrapper skladWrapper = new SkladWrapper(@"..\..\..\..\..\run_1\wms-config.xml", false);
            skladWrapper.AddLogger();
            skladWrapper.AddSklad(timeEnergyMetric);
            skladWrapper.AddAnts(2); //--!
            new MoveSort(skladWrapper).Run(TimeSpan.FromMinutes(5));
            skladWrapper.SaveLog(@"..\..\..\..\..\run_1\log.xml");
            SkladLogger logger = (SkladLogger)skladWrapper.objects.First(x => x is SkladLogger);
            File.WriteAllBytes(@"..\..\..\..\..\run_1\log_unity.xml", SkladWrapper.SerializeXML(logger.logs.ToArray()));
        }
    }
}
